...
void setup() {
  ...
  calibrate();
  Serial.println("calibrated, begin in 1 second");
  delay(1000);
...
void loop() {
  ...
  extruderX = updateExtruderPos(extruderX, xPos, stepPins[0], directionPins[0]);
  extruderY = updateExtruderPos(extruderY, yPos, stepPins[1], directionPins[1]);
}
...
void calibrate(){
  ...
  // Calibration finished, reset extruder tracking
...
int updateExtruderPos(int exPos,int target,int stepPin,int dirPin) {
  // we want to move the extruder towards the input position
  if (exPos > target) {
    moveStepper(stepPin, dirPin, HIGH);
    return(exPos - 1);
  }
  else if (exPos < target) {
    moveStepper(stepPin, dirPin, LOW);
    return(exPos + 1);
  }
}

void moveStepper(int stepPin, int dirPin,int dir) {
